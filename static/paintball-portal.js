(function () {
  ...
  async function loadHostManager() {
    ...
    try {
      ...
      $("hosts-body").innerHTML = players.map((player) => `<tr><td>${escapeHtml(player.in_game_name || "No player profile")}</td><td>${escapeHtml(player.discord_name)}</td><td>${player.is_admin ? "MXS Admin" : player.is_host ? "Approved Host" : "Discord User"}${player.is_premium ? " · Premium" : ""}</td><td><div class="portal-actions">${player.is_admin ? "" : `<button class="btn ${player.is_host ? "ghost" : "secondary"} host-action" data-id="${player.id}" data-action="${player.is_host ? "revoke" : "approve"}">${player.is_host ? "Revoke Host" : "Approve Host"}</button>`}${player.is_host || player.is_admin ? `<button class="btn ${player.is_premium ? "ghost" : "secondary"} host-action" data-id="${player.id}" data-action="${player.is_premium ? "standard" : "premium"}">${player.is_premium ? "Remove Premium" : "Grant Premium"}</button>` : ""}</div></td></tr>`).join("");
